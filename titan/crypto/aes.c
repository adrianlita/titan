...
static void key_expansion(aes_key_size_t key_size, uint8_t *round_key, const uint8_t *key);
...
static void cipher(aes_key_size_t key_size, state_t *state, const uint8_t *round_key);
static void inv_cipher(aes_key_size_t key_size, state_t *state, const uint8_t *round_key);
...
void aes_init_ctx(aes_ctx_t *ctx, aes_key_size_t key_size, const uint8_t *key, const uint8_t *iv) {
    ...
    key_expansion(key_size, ctx->round_key, key);
...
uint32_t aes_pad_buffer_pkcs7(uint8_t *buf, uint32_t length) {
    assert(buf);
    ...
    uint32_t new_length = (length + AES_BLOCKLEN) & 0xFFFFFFF0;
    uint8_t pad = new_length - length;
...
void aes_cbc_encrypt_buffer(aes_ctx_t *ctx, uint8_t *buf, uint32_t length) {
    ...
    for (i = 0; i < length; i += AES_BLOCKLEN) {
        xor_with_iv(buf, iv);
        cipher(ctx->key_size, (state_t*)buf, ctx->round_key);
        iv = buf;
        buf += AES_BLOCKLEN;
    }
...
void aes_cbc_decrypt_buffer(aes_ctx_t *ctx, uint8_t *buf, uint32_t length) {
    ...
    for (i = 0; i < length; i += AES_BLOCKLEN) {
        ...
        inv_cipher(ctx->key_size, (state_t*)buf, ctx->round_key);
